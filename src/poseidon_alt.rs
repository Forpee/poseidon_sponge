...
pub(crate) fn hash_correct<F, A>(p: &mut Poseidon<F, A>) -> F
where
    F: PrimeField,
    A: Arity<F>,
{
...
pub(crate) fn full_round<F, A>(p: &mut Poseidon<F, A>)
where
    F: PrimeField,
    A: Arity<F>,
{
...
pub(crate) fn partial_round<F, A>(p: &mut Poseidon<F, A>)
where
    F: PrimeField,
    A: Arity<F>,
{
...
pub(crate) fn hash_optimized_dynamic<F, A>(p: &mut Poseidon<F, A>) -> F
where
    F: PrimeField,
    A: Arity<F>,
{
...
pub(crate) fn partial_round_dynamic<F, A>(p: &mut Poseidon<F, A>)
where
    F: PrimeField,
    A: Arity<F>,
{
